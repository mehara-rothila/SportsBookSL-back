...
const createDiskStorage = (destinationPath) => {
    ...
    try { ensureDirExists(fullDestinationPath); } catch (err) { console.error(`[Multer EnsureDir] Failed to create directory ${fullDestinationPath}:`, err); }
...
const imageFileFilter = (req, file, cb) => {
    if (!file) { console.warn('[Multer Filter] No file object received by filter.'); return cb(null, true); }
    const filetypes = /jpeg|jpg|png|gif|webp/;
    const extname = filetypes.test(path.extname(file.originalname).toLowerCase());
    const mimetype = filetypes.test(file.mimetype);
    if (mimetype && extname) { return cb(null, true); } else { console.warn(`[Multer Filter] Invalid file type rejected: ${file.originalname} (${file.mimetype})`); req.fileValidationError = new Error('Invalid file type. Only images (jpeg, jpg, png, gif, webp) are allowed.'); cb(null, false); }
};
...
const createUploader = (storage, fileFilter, fieldName, maxCount = 1) => {
    ...
    return (req, res, next) => {
        uploaderMiddleware(req, res, (err) => {
            if (req.fileValidationError) { console.error('[Upload Middleware] File validation error:', req.fileValidationError.message); return res.status(400).json({ message: req.fileValidationError.message }); }
            if (err instanceof multer.MulterError) { console.error('[Upload Middleware] Multer error:', err); let message = `File upload error: ${err.message}`; if (err.code === 'LIMIT_FILE_SIZE') { message = 'File is too large. Maximum size is 10MB.'; } if (err.code === 'LIMIT_UNEXPECTED_FILE') { message = `Unexpected field name '${err.field}'. Check your form data.`; } return res.status(400).json({ message }); }
            if (err) { console.error('[Upload Middleware] Unknown upload error:', err); return res.status(500).json({ message: 'An unexpected error occurred during file upload.' }); }
            next();
        });
    };
};
...
const avatarStorage = createDiskStorage('/uploads/avatars/');
...
const athleteStorage = createDiskStorage('/uploads/athletes/'); // <--- Ensure athlete storage exists
...
module.exports = {
    ...
    uploadAvatar: createUploader(avatarStorage, imageFileFilter, 'avatar'),
    ...
    uploadAthleteImage: createUploader(athleteStorage, imageFileFilter, 'image'), // <--- Ensure athlete uploader uses correct field name
};